 ...
 *
 * Experimental ContextIO plugin
 * (much code cribbed from the StatusNet Sample plugin)
 *
 * Please see the Sample plugin in the main StatusNet repo
 *
 * PHP version 5
 *
 ...
 * @link      http://status.net/
 */
class ContextIOPlugin extends Plugin
{
    /*
     * Some integer that doesn't conflict with the number
     * we gave to other services - Twitter is 1, Facebook is 2.
     *
     * @todo: Fix this crap to be a string or UUID or something
     * @todo: Move these definitions into the Foreign_service object
     *        and allow plugins to extend
     */
    const CONTEXTIO_SERVICE = 33;

    /**
     * Plugins are configured using public instance attributes. To set
     * their values, site administrators use this syntax:
     *
     * addPlugin('ContextIO', array('key' => 'xxxxxx', 'secret' => 'yyyyyyyy'));
     */
    public $key    = null;
    public $secret = null;
    ...
    function onRouterInitialized($m)
    {
        $m->connect(
            'settings/contextio',
            array('action' => 'contextiosettings')
        );
        return true;
    }

    /**
     * Modify the account settings menu to have a ContextIO item
     *
     ...
     * settings menu for the interface to link to our action.
     *
    ...
    function onPluginVersion(&$versions)
    {
        $versions[] = array(
            ...
            'rawdescription' => _m('Use ContextIO with StatusNet')
        );
        return true;
    }
}
